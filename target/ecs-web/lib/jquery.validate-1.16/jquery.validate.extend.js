(function (a) {
    ...
    a.extend(a.validator.defaults, {
        ...
        errorPlacement: function (b, c) {
            ...
            b.insertAfter(c);
            if (c.closest(".mini-textbox-border").length > 0) {
                b.css({
        ...
        unhighlight: function (belement) {         
            a(belement).closest(".form-group").removeClass("has-error");
            if (belement.name == "") {
                a(belement).parent().next().next().remove();
            }            
        },
        success: function (blabel) {
            blabel.remove();
        }
    })
}
