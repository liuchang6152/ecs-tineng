    ...
    var page = {
        ...
        bindUI: function () {
                ...
                if(isTriggerEdit){
                    ...
                    if(e.records.length>1){
                        ...
                        for(var i=0;i<e.records.length;i++){
                            if(e.records[i].isDisabled==1){
                                var check_button = false;
                                //当前这一行，存在于当前gis点关联的所有数据里，那么排除掉；
                                for(var a=0;a<page.data.aDymic_init_dt.length;a++){
                                    if(e.records[i].gisPointForID==page.data.aDymic_init_dt[a].gisPointForID){
                                        check_button = true;
                                        break;
                                    }
                                }
                                if(check_button==false){
                                    //取消选中，剔除其相应的缓存
                                    grid.deselect(e.records[i]);
                                    page.logic.save_selected(e.records[i],false);
                                }
            ...
            $("#btnQuery3").click(function(){
             ...
             //gis点动态数据：点击“已勾选”的复选框
             $('#isSelectMSDS').click(function () {
                var ischecked = $(this).prop("checked");
                if (ischecked) {
                    var checkData = mini.get("datagridMSDS").getSelecteds();
                    //若已勾选
                    $("#datagridMSDS").hide();                     //隐藏表1；
                    $("#datagridMSDS2").show();                    //显示表2；
                    mini.get("datagridMSDS2").clearRows();                         //清除所有行；
                    mini.get("datagridMSDS2").addRows(checkData);   //将选中的行添加进来；
                    mini.get("datagridMSDS2").selectAll();                         //选中所有行；
                }else{
                    //若未勾选
                    page.logic.get_check(gisId,function(){
            ...
            mini.get("datagridMSDS").on("load",function(e){
                var rows= mini.get("datagridMSDS").findRows(function(row){   //找到你要默认选中的行
                          if(row.isband){
                               return true;
                          }
                })
                mini.get("datagridMSDS").selects(rows);
            })
        },
        data: {
            param: {},
        ...
        logic: {
            ...
            */
            ...
            set_init_url:function(n){
                ...
                if(typeof n=="number"){
                    if(n<0){
                        url = searchUrl+"/"+n+"/pageall"+"?type="+$("#type").val();     //查询某个点关联的所有类型的信息
                    }else{
                        url = searchUrl+"/"+n+"/all"+"?type="+$("#type").val();     //查询某个点关联的所有类型的信息
                    }
                }
                if(typeof n=="object"){
                    if(n.type || n.type==""){
                        url = searchUrl+"?type="+n.type;                             //查询某个点的具体某个类型关联的所有信息
                    }
                }
                return url;
            },
            ...
            filter_data:function(dt){
                ...
                if($("#datagridMSDS").is(":visible")){
                    MSDSGridList = mini.get("datagridMSDS").getSelecteds();
                }else{
                    MSDSGridList = mini.get("datagridMSDS2").getSelecteds();
                }
             
                for(var i=0;i<MSDSGridList.length;i++){
                    (function(cur_dt){
                        var cur_one = {};
                        cur_one.gisID = gisId;                                            //gis地图点id;
                        cur_one.type = 5;                                       //gis点的类型;
                        cur_one.gisPointForID = cur_dt.msdsId;                   //gis点的id;
                        cur_one.info = cur_dt.info?cur_dt.info:"";                        //gis点的信息;
                        aCur_data.push(cur_one);
                    })(MSDSGridList[i]);
                }
            ...
            * */
            IntoEdit:function(status,GisId){
                switch (status){
                    case 0:
                        ...
                        $("#isSelectMSDS").prop("checked",false);
                        //表单已勾选不可操作；
                        $("#isSelectMSDS").prop("disabled",true);
                        break;
                    case 1:
                        //进入编辑
                        //表单已勾选处于选中状态；
                        ...
                        $("#isSelectMSDS").prop("checked",true);
                        //表单已勾选可操作；
                        $("#isSelectMSDS").prop("disabled",false);
                        break;
                }
            ...
            get_check:function(GisId,cb){
                ...
                if($("#isSelect").attr("checked")){
                    ...
                    if($("#type").val()==""){
                        ...
                        //未选择“全部”一项的
                        url = page.logic.set_init_url({type:$("#type").val()});
                    }
                }else{
                    //未勾选状态下
                    $("#datagrid").show();      //表格一显示
            ...
            check_dymic_dt:function(cb){
                ...
                if(isTriggerEdit){
                    ...
                    if($("#isSelect").attr("checked")){
                        //若是在勾选状态下
                        url = searchUrl+"/"+gisId+"/all"+"?type="+$("#type").val();     //查询某个点关联的所有类型的信息
                    }else{
                        //若是在非勾选状态下
                        url = searchUrl+"?type="+$("#type").val();                             //查询某个点的具体某个类型关联的所有信息
                    }
                }else{
                    //若是在非编辑状态下；
                    url = searchUrl+"/-1/pageall"+"?type="+$("#type").val();     //查询某个点关联的所有类型的信息
                }
                page.logic.search(url,cb);
            },
            /*
            ...
            * */
            typeClickLoad:function(n,cb){
                ...
                $("#isSelectMSDS").prop("checked",true);    //勾选选中
                $("#isSelectMSDS").prop("disabled",true);    //勾选不可操作
                //查询列表
                var cur_url = "";
                if(n){
                    gisId = n;                       //存储地图的点id;
                    ...
                    cur_url = page.logic.set_init_url({type:""});
                }
            ...
            * */
            deletes: function(GisId){
                if (gisId){
                    var deleteUrl = searchUrl+"/"+gisId+"/all";
                    $.ajax({
                        url: deleteUrl,
                        async: true,
                        dataType: "text",
            ...
            save: function (GisId) {
                ...
                if (gisId>0) {
                    ...
                    var ajaxType = "POST";
                    //拼接数据----------
                    var jsonArr = page.logic.filter_data(page.data.aSelected_dt);
                    //当前gis点关联的动态数据存在的情况下，向后端提交数据；
                    $.ajax({
                        url: searchUrl + "/" + gisId + "/all?enterprise="+ECS.sys.Context.SYS_ENTERPRISE_CODE,
                        type: ajaxType,
            ...
            save_success:function(result_dt){
                ...
                if($("#datagridMSDS").is(":visible")){
                    MSDSList = mini.get("datagridMSDS").getSelecteds();
                }else{
                    MSDSList = mini.get("datagridMSDS2").getSelecteds();
                }
