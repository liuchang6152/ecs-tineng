...
var searchUrl = ECS.api.rttUrl + '/medicalInstitutio/getMedicalAddressPage'; //接口
		...
		logic: {
			...
			*/
			...
			set_init_url: function (n) {
				...
				if (typeof n == "number") {
					if (n < 0) {
						url = searchUrl + "/" + n + "/pageall" + "?status=" + ''; //查询某个点关联的所有类型的信息
					} else {
						url = searchUrl + "/" + n + "/all" + "?status=" + ''; //查询某个点关联的所有类型的信息
					};
				};
				if (typeof n == "object") {
					if (n.type != undefined) {
			...
			* */
			...
			check_dymic_dt: function (cb) {
				...
				if (isTriggerEdit) {
					...
					if ($("#isSelect").attr("checked")) {
						//若是在勾选状态下
						url = searchUrl + "/" + gisId + "/all" + "?status=" + $("#name").val(); //查询某个点关联的所有类型的信息
					} else {
						//若是在非勾选状态下
						url = searchUrl + "?status=" + $("#name").val(); //查询某个点的具体某个类型关联的所有信息
					}
				} else {
					//若是在非编辑状态下；
					url = searchUrl + "?status=1" + "&medicalName=" + $("#name").val(); //查询某个点关联的所有类型的信息
				}
				page.logic.search(url, cb);
			},
			/*
			...
			deletes: function (GisId) {
				if (gisId) {
					var deleteUrl = ECS.api.medicalGis + '/delmedicalGis?gisId=' + GisId + '&systemId=' + page.data.aSelected_dt[0].institutionId;
					$.ajax({
						url: deleteUrl,
						async: false,
						dataType: "json",
			...
			save: function (GisId) {
				...
				if (gisId > 0) {
					...
					var jsonArr = {
						"gisId": GisId,
						"systemId": page.data.aSelected_dt[0].institutionId,
					};
					var obj = {
						"institutionId": page.data.aSelected_dt[0].institutionId || '',
						"medicalName": page.data.aSelected_dt[0].survVideoName || ''
					}
					//当前gis点关联的动态数据存在的情况下，向后端提交数据；
					$.ajax({
						url: ECS.api.medicalGis + '/addMedicalGis',
						type: ajaxType,
			...
			give_up: function () {
				...
				grid4.load(grid4Obj, function (result) {
					// grid4.selectAll(true);
					// cb && cb();
				});
				grid.hide();
				...
				grid4.show();
				$("#isSelect").prop("checked", true); //勾选选中
				$("#isSelect").prop("disabled", true); //放弃不可操作
				// page.logic.check_point(-1); //刷新动态数据和静态数据；（初始化，所有点的关联的数据）
			},
