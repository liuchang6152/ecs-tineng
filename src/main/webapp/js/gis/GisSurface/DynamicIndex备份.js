...
var searchUrl = ECS.api.gisSurfaceUrl;
...
/*            else {
        ...
        bindUI: function () {
            ...
            grid.on("select",function (e) {
                ...
                var querResult = JSLINQ(data).Where(function (x) { return  x.gisSurfaceForID != row.gisSurfaceForID ; }).ToArray();
...
/*            grid.on("deselect",function (e) {
                var row = e.record;
                var data = grid2.getData();
                var querResult = JSLINQ(data).Where(function (x) { return x.surfaceID == row.surfaceID && x.gisSurfaceForID == row.gisSurfaceForID && x.orgLvl == row.orgLvl ; }).ToArray();
                grid2.removeRows(querResult, true);
                page.logic.setGrid2Selecteds();
            });*/
/*            grid2.on("deselect",function (e) {
                grid = mini.get("datagrid");
                var rowt = e.record;
                var rows=grid.findRows(function (row) {

                    if (row.gisSurfaceForID==rowt.gisSurfaceForID)
                    {
                        return true;
                    }
                    else
        ...
        logic: {
            ...
            * */
            getDatagrid2RUL:function(gisSurfaceId){
                ...
                switch (typeCode) {
                    case 1:
                        datagrid2URL=searchUrl+"/"+gisSurfaceId+"/org/"+1;
                        break;
                    case 2:
            ...
            getSave2RUL:function(gisSurfaceId,typeValueCode){
                ...
                if (typeValueCode !=0 && typeValueCode !=null) {
                    switch (typeValueCode) {
                        case 1:
                            save2URL=searchUrl+"/"+gisSurfaceId+"/org/"+1+"?enterprise="+enterpriseCode;
                            break;
                        case 2:
            ...
            * 点击类型类型加载
            * */
            typeClickLoad:function(typeCodeValue,stateValue){
                mini.parse();
